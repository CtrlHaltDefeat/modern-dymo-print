...
import { findAllWebServices, findWebService } from "@/webService/findWebService.ts";
...
Deno.test("findWebService - no service running", async () => {
	...
	await assertRejects(() => findWebService(), Error, NO_WEB_SERVICE_FOUND_ERROR);
});
	...
	fn: async (t) => {
		...
		for (const serviceHost of hosts) {
			for (const servicePort of portRange) {
				await t.step(`should find service on host ${serviceHost} port ${servicePort}`, async () => {
					const server = await startHttpServer(serviceHost, servicePort, (request, _info) => {
						if (!request.url.includes(serviceHost)) return new Response("Not found", { status: 404 });
