...
import DymoLabel from "@/label/DymoLabel";
import { setLoggingState } from "@/logging/enableLogging";
import logMessage, { LogLevel } from "@/logging/logMessage";
import type DymoPrinter from "@/printers/DymoPrinter";
import DZPrinter from "@/printers/DZPrinter";
import LabelWriter from "@/printers/LabelWriter";
import TapePrinter from "@/printers/TapePrinter";
import { findWebService } from "@/webService/findWebService";
import invokeCommand, { HttpMethod } from "@/webService/invokeCommand";
...
import { FAILED_TO_PRINT_LABEL_ERROR, FAILED_TO_RENDER_LABEL_ERROR, PARSE_PRINTERS_ERROR, PRINTER_NOT_CONNECTED_ERROR } from "./constants";
...
export default class DymoPrintService {
	...
	public async printLabel(printer: DymoPrinter, label: DymoLabel | string, printParamsXml?: string, labelSetXml?: string): Promise<void> {
		...
		try {
			...
			logMessage(FAILED_TO_PRINT_LABEL_ERROR, undefined, LogLevel.ERROR);
			throw new Error(FAILED_TO_PRINT_LABEL_ERROR);
		}
	}
